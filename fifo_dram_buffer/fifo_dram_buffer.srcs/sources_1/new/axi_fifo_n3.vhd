...
architecture Behavioral of axi_fifo_n3 is
  ...
  signal axi_read_cycle_count   : natural := 0;
  signal axi_write_cycle_count  : natural := 0;
  signal axi_total_read_cycles  : natural := 0;
  signal axi_total_write_cycles : natural := 0;
  signal axi_num_reads          : natural := 0;
  signal axi_num_writes         : natural := 0;
  signal axi_max_read_cycles    : natural := 0;
  signal axi_max_write_cycles   : natural := 0;
  signal axi_min_read_cycles    : natural := natural'high;
  signal axi_min_write_cycles   : natural := natural'high;
  signal axi_avg_read_cycles    : natural := 0;
  signal axi_avg_write_cycles   : natural := 0;
begin
  ...
  -- Modify FSM processes to count cycles
    ...
    begin
        if buffer_rst = '0' then
            ...
            axi_read_cycle_count   <= 0;
            axi_write_cycle_count  <= 0;
            axi_total_read_cycles  <= 0;
            axi_total_write_cycles <= 0;
            axi_num_reads          <= 0;
            axi_num_writes         <= 0;
            axi_max_read_cycles    <= 0;
            axi_max_write_cycles   <= 0;
            axi_min_read_cycles    <= natural'high;
            axi_min_write_cycles   <= natural'high;
            axi_avg_read_cycles    <= 0;
            axi_avg_write_cycles   <= 0;
        elsif rising_edge(buffer_clk) then
            ...
            elsif mainFSM_currentState = FSM_RECEIVE_HS then
                write_cycle_count <= write_cycle_count + 1;
            end if;
            
            if mainFSM_currentState = FSM_IDLE and mainFSM_prevState = FSM_RECEIVE_AXI then
            ...
            if mainFSM_currentState = FSM_IDLE and mainFSM_prevState = FSM_SEND_HS then
            ...
            if axiFSM_currentState = AXI_FSM_WAIT_FOR_READ_ACK1 or (axiFSM_currentState = AXI_FSM_WAIT_FOR_READ_CMPLT1 and bus2ip_mst_cmplt = '0') then
               axi_read_cycle_count <= axi_read_cycle_count + 1;
            end if;
            
            if axiFSM_currentState = AXI_FSM_WAIT_FOR_WRITE_ACK1 or (axiFSM_currentState = AXI_FSM_WAIT_FOR_WRITE_CMPLT1 and bus2ip_mst_cmplt = '0') then
               axi_write_cycle_count <= axi_write_cycle_count + 1;
            end if;
    
            if axiFSM_currentState = AXI_FSM_WAIT_FOR_READ_CMPLT1 and bus2ip_mst_cmplt = '1' then
                axi_total_read_cycles <= axi_total_read_cycles + axi_read_cycle_count;
                axi_num_reads         <= axi_num_reads + 1;
                if axi_read_cycle_count > axi_max_read_cycles then
                    axi_max_read_cycles <= axi_read_cycle_count;
                end if;
                if axi_read_cycle_count < axi_min_read_cycles then
                    axi_min_read_cycles <= axi_read_cycle_count;
                end if;
                if axi_num_reads > 0 then
                    axi_avg_read_cycles <= axi_total_read_cycles / axi_num_reads;
                end if;
                axi_read_cycle_count <= 0;
            end if;
    
            if axiFSM_currentState = AXI_FSM_WAIT_FOR_WRITE_CMPLT1 and bus2ip_mst_cmplt = '1' then
                axi_total_write_cycles <= axi_total_write_cycles + axi_write_cycle_count;
                axi_num_writes         <= axi_num_writes + 1;
                if axi_write_cycle_count > axi_max_write_cycles then
                    axi_max_write_cycles <= axi_write_cycle_count;
                end if;
                if axi_write_cycle_count < axi_min_write_cycles then
                    axi_min_write_cycles <= axi_write_cycle_count;
                end if;
                if axi_num_writes > 0 then
                    axi_avg_write_cycles <= axi_total_write_cycles / axi_num_writes;
                end if;
                axi_write_cycle_count <= 0;
            end if;
        end if;
        ...
        avg_read_cycles_out  <= std_logic_vector(to_unsigned(avg_read_cycles,13));
        avg_write_cycles_out <= std_logic_vector(to_unsigned(avg_write_cycles, 13));
        max_read_cycles_out  <= std_logic_vector(to_unsigned(max_read_cycles, 13));
        max_write_cycles_out <= std_logic_vector(to_unsigned(max_write_cycles, 13));
        min_read_cycles_out  <= std_logic_vector(to_unsigned(min_read_cycles, 13));
        min_write_cycles_out <= std_logic_vector(to_unsigned(min_write_cycles, 13));
        
        axi_avg_read_cycles_out  <= std_logic_vector(to_unsigned(axi_avg_read_cycles, 13));
        axi_avg_write_cycles_out <= std_logic_vector(to_unsigned(axi_avg_write_cycles, 13));
        axi_max_read_cycles_out  <= std_logic_vector(to_unsigned(axi_max_read_cycles, 13));
        axi_max_write_cycles_out <= std_logic_vector(to_unsigned(axi_max_write_cycles, 13));
        axi_min_read_cycles_out  <= std_logic_vector(to_unsigned(axi_min_read_cycles, 13));
        axi_min_write_cycles_out <= std_logic_vector(to_unsigned(axi_min_write_cycles, 13));
    end process;
