...
end input_selector_78;


architecture Behavioral of input_selector_78 is
...
begin
  ...
  begin
    if falling_edge(clk) then
        ...
        if (op_in_valid_56 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 57) then
        if (op_in_valid_57 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 58) then
        if (op_in_valid_58 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 59) then
        if (op_in_valid_59 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 60) then
        if (op_in_valid_60 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 61) then
        if (op_in_valid_61 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 62) then
        if (op_in_valid_62 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 63) then
        if (op_in_valid_63 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 64) then
        if (op_in_valid_64 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 65) then
        if (op_in_valid_65 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 66) then
        if (op_in_valid_66 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 67) then
        if (op_in_valid_67 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 68) then
        if (op_in_valid_68 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 69) then
        if (op_in_valid_69 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 70) then
        if (op_in_valid_70 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 71) then
        if (op_in_valid_71 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 72) then
        if (op_in_valid_72 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 73) then
        if (op_in_valid_73 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 74) then
        if (op_in_valid_74 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 75) then
        if (op_in_valid_75 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 76) then
        if (op_in_valid_76 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
elsif (current_phase = 77) then
        if (op_in_valid_77 = '1' and op_out_ready_0 = '1') then
            next_phase <= (current_phase + 1) mod num_phases;
        end if;
    end if;
...
op_in_valid_56 when (current_phase = 56) else
op_in_valid_57 when (current_phase = 57) else
op_in_valid_58 when (current_phase = 58) else
op_in_valid_59 when (current_phase = 59) else
op_in_valid_60 when (current_phase = 60) else
op_in_valid_61 when (current_phase = 61) else
op_in_valid_62 when (current_phase = 62) else
op_in_valid_63 when (current_phase = 63) else
op_in_valid_64 when (current_phase = 64) else
op_in_valid_65 when (current_phase = 65) else
op_in_valid_66 when (current_phase = 66) else
op_in_valid_67 when (current_phase = 67) else
op_in_valid_68 when (current_phase = 68) else
op_in_valid_69 when (current_phase = 69) else
op_in_valid_70 when (current_phase = 70) else
op_in_valid_71 when (current_phase = 71) else
op_in_valid_72 when (current_phase = 72) else
op_in_valid_73 when (current_phase = 73) else
op_in_valid_74 when (current_phase = 74) else
op_in_valid_75 when (current_phase = 75) else
op_in_valid_76 when (current_phase = 76) else
op_in_valid_77 when (current_phase = 77) else
'0';
...
op_in_data_56 when (current_phase = 56) else
op_in_data_57 when (current_phase = 57) else
op_in_data_58 when (current_phase = 58) else
op_in_data_59 when (current_phase = 59) else
op_in_data_60 when (current_phase = 60) else
op_in_data_61 when (current_phase = 61) else
op_in_data_62 when (current_phase = 62) else
op_in_data_63 when (current_phase = 63) else
op_in_data_64 when (current_phase = 64) else
op_in_data_65 when (current_phase = 65) else
op_in_data_66 when (current_phase = 66) else
op_in_data_67 when (current_phase = 67) else
op_in_data_68 when (current_phase = 68) else
op_in_data_69 when (current_phase = 69) else
op_in_data_70 when (current_phase = 70) else
op_in_data_71 when (current_phase = 71) else
op_in_data_72 when (current_phase = 72) else
op_in_data_73 when (current_phase = 73) else
op_in_data_74 when (current_phase = 74) else
op_in_data_75 when (current_phase = 75) else
op_in_data_76 when (current_phase = 76) else
op_in_data_77 when (current_phase = 77) else
...
'0';
op_in_ready_56 <= op_out_ready_0 when (current_phase = 56) else
'0';
op_in_ready_57 <= op_out_ready_0 when (current_phase = 57) else
'0';
op_in_ready_58 <= op_out_ready_0 when (current_phase = 58) else
'0';
op_in_ready_59 <= op_out_ready_0 when (current_phase = 59) else
'0';
op_in_ready_60 <= op_out_ready_0 when (current_phase = 60) else
'0';
op_in_ready_61 <= op_out_ready_0 when (current_phase = 61) else
'0';
op_in_ready_62 <= op_out_ready_0 when (current_phase = 62) else
'0';
op_in_ready_63 <= op_out_ready_0 when (current_phase = 63) else
'0';
op_in_ready_64 <= op_out_ready_0 when (current_phase = 64) else
'0';
op_in_ready_65 <= op_out_ready_0 when (current_phase = 65) else
'0';
op_in_ready_66 <= op_out_ready_0 when (current_phase = 66) else
'0';
op_in_ready_67 <= op_out_ready_0 when (current_phase = 67) else
'0';
op_in_ready_68 <= op_out_ready_0 when (current_phase = 68) else
'0';
op_in_ready_69 <= op_out_ready_0 when (current_phase = 69) else
'0';
op_in_ready_70 <= op_out_ready_0 when (current_phase = 70) else
'0';
op_in_ready_71 <= op_out_ready_0 when (current_phase = 71) else
'0';
op_in_ready_72 <= op_out_ready_0 when (current_phase = 72) else
'0';
op_in_ready_73 <= op_out_ready_0 when (current_phase = 73) else
'0';
op_in_ready_74 <= op_out_ready_0 when (current_phase = 74) else
'0';
op_in_ready_75 <= op_out_ready_0 when (current_phase = 75) else
'0';
op_in_ready_76 <= op_out_ready_0 when (current_phase = 76) else
'0';
op_in_ready_77 <= op_out_ready_0 when (current_phase = 77) else
'0';
